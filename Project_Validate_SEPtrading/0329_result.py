...
class MyPlay(trade.Model):
    def on_initialize(self):
        ...
        self.df = DataFrame(columns= ['date','history','order0','order1','order2','order3','order4','order5','bar0','bar1','bar2','bar3','bar4','bar5','t0','t1','t2','t3','t4','t5','Trade_date','date0','total_volumn','total_size','average_price','average_price_once','price1','price2','price3','price4','price5'],index = self.list['name'])
        ...
        for instr in self.list['name']:
            ...
            self.report['report'][instr]['possible_error1'] = []   #记录可能出现的错误:bar线数据与实际成交数据差别太大。
